    ...
    Tag: Clone + std::fmt::Debug,
{
    ...
    async fn get_item_refs_for_phrase(&self, phrase: &str) -> Result<Vec<ItemRef>, DbError> {
    ...
    async fn get_item_refs_search_query(
        &self,
        search_query: &str,
        word_max: usize,
        phrase_max: usize,
    ) -> Result<Vec<ItemRef>, DbError> {
        ...
        let listings =
            try_join_all(to_process_phrases.into_iter().flatten().map(
