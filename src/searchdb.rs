...
use crate::schemas::RecipeStore;
use crate::search::{ItemRepo, SearchDb};
use crate::{
    schemas::{AppError, Lang},
    search::SearchCache,
};
...
impl SearchDb<String, String, RecipeStore, AppError> for RepositoryDb {
    ...
    async fn get_item_from_ref(&self, slug: String) -> Result<RecipeStore, AppError> {
        let key = format!("recipe.{slug}");
...
impl ItemRepo<String, String, RecipeStore, AppError> for Repository {
    fn get_cache(&self) -> impl SearchCache<String, AppError> {
        self.redis.clone()
    }

    fn get_db(&self) -> impl SearchDb<String, String, RecipeStore, AppError> {
        self.redka.clone()
    }
}
